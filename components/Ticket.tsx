...
export default function Ticket() {
  ...
  async function getTickets() {
    let { data: updatedTickets, error } = await supabase.from('tickets').select('*');
  ...
  return loading || !user ? (
    ...
    sorted
      .map((ticket: any) => {
        return (
          <div
            style={{ width: '100%', display: 'flex', flexDirection: 'column', margin: '0 0 40px 0' }}
            key={ticket.id}
          >
            <EmergencyTicket
              ...
              priorityLevelTitleStyles={priorityLevelTitleStyles}
    ...
    <div style={{ display: 'flex', alignItems: 'center', flexDirection: 'column', margin: '100px 0' }}>
      ...
      <button className='redirectButton' onClick={() => router.push('/')}>
        Submit a ticket
      </button>
    </div>
  );
...
function EmergencyTicket({
  ...
  tickets: any;
  mediaQueries: any;
  priorityLevelTitleStyles: CSSProperties;
  handleClaimTicket: Function;
  user: any;
  ...
  return tickets
    .map((ticket: any, index: number) => {
      if (ticket.priority_level === 3) {
        ...
        return (
          <p key={index} style={priorityLevelTitleStyles}>
            <span style={{ color: palette.emergencyRed, textDecoration: 'underline' }}>EMERGENCY TICKETS</span>
          </p>
        );
      }
    })
  ...
  return tickets
    .map((ticket: any, index: number) => {
      if (ticket.priority_level === 2) {
        ...
        return (
          <p key={index} style={priorityLevelTitleStyles}>
            Priority Tickets, need to be completed ASAP
          </p>
        );
      }
    })
  ...
  return tickets
    .map((ticket: any, index: number) => {
      if (ticket.priority_level === 1) {
        ...
        return (
          <p key={index} style={priorityLevelTitleStyles}>
            Need to be completed by the end of the week
          </p>
        );
      }
    })
...
function ActualTicket({
  ...
  return ticket.priority_level === setPriorityLevel ? (
    <div className='ticketList-ticket' key={ticket.id}>
      ...
      >
        <div style={{ position: 'relative', padding: '20px', width: '100%' }}>
          {user.user_metadata.typeOfUser === 'admin' ? (
            <p
              style={{
                margin: 0,
                right: editing && mediaQueries.under768 ? 0 : editing ? 120 : 0,
                top: 20,
                position: 'absolute',
          ...
          {user.email === ticket.created_by ? (
            <button
              ...
              style={{
                left: editing && mediaQueries.under768 ? 0 : editing ? 120 : 0,
                top: 20,
                position: 'absolute',
                margin: 0,
                padding: '7px 14px',
        ...
        {editing ? (
          <form style={{ border: 'none', width: '100%', height: '500px' }}>
            <input
              ...
              style={{ width: mediaQueries.under768 ? '240px' : '400px', margin: '0 0 20px 0' }}
            />
            <textarea
            ...
            >
              {submittingEdits ? 'Submitting...' : 'Submit'}
            </button>
          </form>
        ) : (
          <>
            <p style={{ fontWeight: 700, fontSize: 24, textAlign: 'center', wordBreak: 'break-all' }}>{ticket.title}</p>
            <p style={{ fontWeight: 500, fontSize: 18, textAlign: 'center', wordBreak: 'break-word' }}>
              {ticket.description}
            </p>
        ...
        {user.user_metadata.typeOfUser === 'admin' && !ticket.assigned_to ? (
          <button onClick={(e) => handleClaimTicket(e, ticket, setClaiming, setClaimed)}>
            {claiming ? 'Claiming...' : claimed ? 'Claimed!' : 'Claim'}
          </button>
        ) : (
          <p style={{ fontWeight: 500, fontSize: 18, textAlign: 'center', margin: '0 0 6px 0' }}>
            {ticket.assigned_to ? `Assigned to: ${ticket.assigned_to}` : 'Not yet assigned'}
          </p>
        )}
        {ticket.reviewed_by ? (
          <p style={{ fontWeight: 500, fontSize: 18, textAlign: 'center', margin: '0 0 12px 0' }}>
            Reviewed by: {ticket.reviewed_by}
          </p>
        ) : null}
      </div>
    </div>
