...
export default function HamburgerMenu({ hamburgerClick, setHamburgerClick, backgroundColor, color }: { hamburgerClick: boolean, setHamburgerClick: (value: boolean) => void, backgroundColor?: string, color?: string }) {
  ...
  return (
    hamburgerClick ?
      ...
      >
        <div style={{ width: "80%", backgroundColor: backgroundColor ? backgroundColor : "#c2c2c2", height: "100%", color: color ? color : "black" }}>
          <p onClick={() => setHamburgerClick(false)} style={{ margin: "3% 0 0 87%", padding: "10px", cursor: "pointer" }}>X</p>
          <div style={{ margin: "1% 0 0 5%" }}>
            <p style={{ cursor: "pointer" }} onClick={() => router.push("/")}>Ticket Form</p>
            <p style={{ cursor: "pointer" }} onClick={() => router.push("/ticket-list")}>Ticket List</p>
            {admin ?
              <p style={{ cursor: "pointer" }} onClick={() => router.push("/dev-tickets")}>Developer Tickets</p>
              : null}
            {admin ?
