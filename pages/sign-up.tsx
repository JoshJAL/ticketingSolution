...
export default function SignUp() {
  ...
  async function fetchGeneralUserEmails() {
    let { data: users, error } = await supabase.from('generalUsers').select('*');
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100vh' }}>
      ...
      <form style={{ border: mediaQueries.under768 ? 'none' : '1px solid black', borderRadius: '10px' }}>
        {verify ? (
          <p style={{ textAlign: 'center', margin: '20px' }}>{'Please check your email and verify your account!'}</p>
        ) : (
          <>
            {isCapsLockOn ? (
              <p style={{ color: palette.emergencyRed, fontWeight: 700 }}>Warning: Caps Lock is on!</p>
            ) : null}
            <label style={{ textAlign: 'center', color: notAllowed ? palette.emergencyRed : 'black' }}>
              {notAllowed ? 'You are not authorized to create an account with that email' : 'Create an account!'}
            </label>
